...
import { formatUSDT, formatPrice, formatTokens, formatPct, formatTime } from "@/lib/format";
import { explorerTx, shortAddr } from "@/lib/rpc";
import {
  Activity, Search, X, TrendingUp, TrendingDown, ExternalLink,
} from "lucide-react";
...
export function ChartTab({ tokenAddress, onPriceUpdate }: ChartTabProps) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      timeScale: { borderColor: "#1a1a24", timeVisible: true, secondsVisible: false },
      rightPriceScale: { borderColor: "#1a1a24" },
      localization: {
        priceFormatter: (p: number) =>
          p >= 1_000_000 ? `$${(p / 1_000_000).toFixed(2)}M`
          : p >= 1_000   ? `$${(p / 1_000).toFixed(1)}K`
          : `$${p.toFixed(2)}`,
      },
    });
  ...
  useEffect(() => {
    ...
    const filled      = fillCandles(tradesToCandles(allTrades, timeframe), timeframe);
  ...
  useEffect(() => {
    ...
    const markerList = [
      ...
      ...stats.sells.map((t) => ({
        ...
        color: "#ef4444", shape: "arrowDown" as const,
  ...
  // ── Recalc stats on price tick ────────────────────────────────────────────
  useEffect(() => {
    if (!trackedWallet || allTrades.length === 0) return;
    setWalletStats(calcWalletStats(trackedWallet, allTrades, latestPrice));
  }, [latestPrice]);

  const trackWallet = useCallback((addr: string) => {
    const v = addr.trim();
    if (!v) return;
    setWalletInput(v);
    setTrackedWallet(v);
    setWalletStats(calcWalletStats(v, allTradesRef.current, latestPrice));
  }, [latestPrice]);
  ...
  // Sorted newest-first for the trades table
  const tradesDesc = [...allTrades].sort((a, b) => b.ts - a.ts);
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-2xl font-mono font-bold text-accent">{formatUSDT(marketCap, true)}</div>
        </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && trackWallet(walletInput)}
            placeholder="Track wallet: 0x...  (or click any wallet below)"
        ...
        <button
          onClick={() => trackWallet(walletInput)}
        ...
        {trackedWallet && (
          <button onClick={clearWallet} className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors" title="Clear">
            <X className="w-4 h-4" />
          </button>
        )}
      </div>
      ...
      {/* ── Chart ── */}
      <div ref={chartContainerRef} className="w-full border border-border rounded-lg overflow-hidden" />
        ...
        <div className="border border-border rounded-lg overflow-hidden">
          <div className="bg-muted/40 border-b border-border px-4 py-2 flex items-center justify-between">
            ...
            <div className={`flex items-center gap-1 text-sm font-mono font-bold ${pnlPositive ? "text-green-500" : "text-red-500"}`}>
              ...
              <span className="text-xs opacity-70 ml-1">({formatPct(walletStats.totalPnlPct)})</span>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-border">
            {[
              { label: "INVESTED",  val: formatUSDT(walletStats.investedUsdt, true),  sub: `${walletStats.buys.length} buy${walletStats.buys.length !== 1 ? "s" : ""}` },
              { label: "RECEIVED",  val: formatUSDT(walletStats.receivedUsdt, true),  sub: `${walletStats.sells.length} sell${walletStats.sells.length !== 1 ? "s" : ""}` },
              { label: "HOLDING",   val: formatTokens(walletStats.netTokens),         sub: `≈ ${formatUSDT(walletStats.currentValue, true)}` },
              { label: "TOTAL P&L", val: (pnlPositive ? "+" : "") + formatUSDT(walletStats.totalPnl, true), sub: formatPct(walletStats.totalPnlPct), color: pnlPositive ? "text-green-500" : "text-red-500" },
              ...
              <div key={label} className="px-4 py-3">
                ...
                <div className={`text-[10px] font-mono mt-0.5 ${color ? color + "/70" : "text-muted-foreground"}`}>{sub}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {trackedWallet && walletStats && walletStats.buys.length === 0 && walletStats.sells.length === 0 && (
        <div className="border border-border/50 rounded-lg p-4 text-center">
          <p className="text-xs font-mono text-muted-foreground">No trades found for this wallet</p>
        </div>
      )}
          ...
          <table className="w-full">
            <thead className="sticky top-0 bg-card border-b border-border z-10">
              <tr className="text-[10px] font-mono text-muted-foreground">
                ...
                <th className="text-right px-3 py-2">USDT</th>
                <th className="text-right px-3 py-2">MC</th>
            ...
            <tbody>
              {tradesDesc.map((trade, i) => {
                const isTracked = trackedWallet &&
                  trade.trader.toLowerCase() === trackedWallet.toLowerCase();
                return (
                  <tr
                    key={`${trade.txHash}-${i}`}
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right font-mono text-[11px] font-semibold whitespace-nowrap">
                      {formatUSDT(trade.usdt, true)}
                    </td>
                    <td className="px-3 py-2 text-right font-mono text-[11px] text-muted-foreground whitespace-nowrap">
                      {formatUSDT(trade.price * SUPPLY, true)}
                    </td>
                    <td className="px-3 py-2 text-right font-mono text-[11px] text-muted-foreground whitespace-nowrap">
                    ...
                    <td className="px-3 py-2">
                      <button
                        onClick={() => trackWallet(trade.trader)}
                        title="Click to track this wallet"
                        className={`font-mono text-[11px] transition-colors hover:underline ${
                          isTracked ? "text-accent font-bold" : "text-accent/70 hover:text-accent"
                        }`}
                      >
